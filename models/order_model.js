...
class Order {
   ...
   async  getOrderHistoryByIdForShop(shop_id) {
    ...
    async createNewOrder(id, buyer_id, shop_id, message_body, started, last_updated_timestamp, total_price, items_list) {
        ...
        let query2 = 'INSERT INTO public.order_item (order_id, item_id, amount, price) values (($1),($2),($3),($4))';
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (result.rowCount === 1) {
                    ...
                    for (let i = 0; i < items_list.length; i++) {
                        prom_list.push(pg_pool.query(query2, [items_list[i].order_id, items_list[i].item_id, items_list[i].amount, items_list[i].price]));
                    }
                    Promise.all(prom_list).then((rs) => {
                        let bool_val = true;
